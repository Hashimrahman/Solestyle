...
const Register = () => {
  ...
  const [errors, setError] = useState({}); // State for error messages
  const [valid, setValid] = useState(true); // State for form validity status
  ...
  const handleSubmit = async (e) => {
    ...
    // Check if password is entered and have the minimum length
    ...
    setError(validationErrors); // Update error
    setValid(isValid); // Update validity status of form
    if (!isValid) return; // if validation fails, then exit
