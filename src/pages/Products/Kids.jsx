...
const Kids = () => {
  ...
  const { kids } = useContext(ProductContext);
      ...
      <div className="mt-20 text-center relative">
        ...
        <div className="flex flex-wrap mt-8 items-center justify-center">
          {kids.map((kids) => (
            ...
            >
              <ProductCard kids={kids} navigate={navigate} />
            </div>
          ))}
        </div>
      </div>
...
const ProductCard = ({ kids, navigate }) => {
  ...
  return (
    <div className="relative group border-[2px] p-4 w-full text-left rounded-md">
      ...
      <h3 className="opacity-60 uppercase">{kids.brand}</h3>
      <h2 className="mt-4 text-lg font-semibold">{kids.name}</h2>
      <p className="text-md font-medium text-gray-700">${kids.price}</p>
      <div className="flex justify-center">
        <button
          ...
          onClick={() => navigate(`/product/${kids.id}`)}
        >
          View Product
        </button>
      </div>
